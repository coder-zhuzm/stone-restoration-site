...
本文档记录石像与佛塔修复场景的真实实现过程，并定义后续新增其他佛塔、石砖、石碑、造像和建筑构件修复场景时必须遵守的素材协议、代码结构、生成提示词、验收方式与交接格式。
...
已足够支撑两个可运行修复场景，但仍不足以支撑多个视觉上完全独立的章节。

现有素材能够支撑石像与佛塔两个主修复对象。佛塔已经具备同源完整态和残缺态，因此可以使用公共引擎完成第二个修复流程。

当前限制是佛塔场景仍复用石像场景的远景、前景和中景语言，只替换主物件、缺损范围、构图参数、碎片配色和文案。因此它是“第二个可运行修复场景”，还不是“第二套完全独立美术场景”。若新增第三个主物件，仍必须先补齐该物件的同源完整态和残缺态。
...
| 本次版本 | 生成五层佛塔三项素材，参数化缺损区域，建立石像／佛塔场景注册表并接入第二场景 |
...
| `midground-pagoda.png` | `1024×1536` | 是 | 石像 | 中景装饰 | 可在其他场景作为远／中景装饰，不可作为修复主物件 |
...
当前版本采用同等能力的轻量结构：`src/sceneConfig.ts` 同时保存类型、注册表和两个场景配置，`src/main.ts` 只保留一套公共循环。场景继续增加到 4 个以上时，再按上面的目标目录拆分为独立文件，避免提前制造多层抽象。
...
| 2 | 佛塔修复 | 正立面砖石佛塔 | 塔刹、檐角、砖墙补全 | 最高 |
| 3 | 石砖修复 | 残损城墙／经砖 | 缺砖归位与裂缝闭合 | 高 |
| 4 | 石碑修复 | 断裂碑身 | 断口拼合与刻纹显现 | 中 |
| 5 | 浮雕修复 | 佛教浅浮雕 | 局部轮廓和纹样补全 | 中 |
...
3. 如需让佛塔成为完全独立章节，生成佛塔专属远景、中景和前景。
4. 如需使用图像碎片替代程序化碎片，将 `fragments.png` 输出为单件文件或建立 UV 图集配置。
5. 为第三个场景生成同源完整态／残缺态，再仅新增场景配置。
6. 场景达到 4 个以上时，将 `sceneConfig.ts` 拆分为注册表与独立场景模块。
