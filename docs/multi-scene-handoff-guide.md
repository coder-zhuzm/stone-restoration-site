...
本文档记录当前石像修复场景的真实实现过程，并定义后续新增佛塔、石砖、石碑、造像和建筑构件修复场景时必须遵守的素材协议、代码结构、生成提示词、验收方式与交接格式。
...
现有素材能够完整支撑当前石像场景，也可以在同一遗迹世界内复用背景、地形和中景装饰物，但不能直接构成第二个独立修复场景。

原因：每个新场景必须拥有该场景主物件的同源完整态、残缺态和缺损范围；当前只有石像具备这组核心素材。
