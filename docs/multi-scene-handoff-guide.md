...
已足够支撑两个视觉上独立且可运行的修复场景。

现有素材能够支撑石像与佛塔两个主修复对象。佛塔已经具备同源完整态和残缺态，因此可以使用公共引擎完成第二个修复流程。

石像与佛塔拥有各自的远景、中景、前景、主物件、缺损范围、构图参数、碎片配色和文案，只复用公共渲染与交互引擎。若新增第三个主物件，仍必须先补齐该物件的同源完整态、残缺态和至少一套能建立自身场景语义的环境素材。
...
| 本次版本 | 生成并接入佛塔专属远景、中景和前景，使两个场景形成独立美术章节 |
