...
> 状态：当前三个独立场景的最终采用素材与提示词记录。它记录“实际用了什么”，不是未来连续世界的素材清单；通用生产与改图方法见 [素材生产指南](asset-production-guide.md)，连续世界新增素材要求见 [连续世界方案](continuous-world-plan.md)。
...
早期素材的精确底层模型快照、seed、全部失败样张、调用次数和成本没有完整记录，因此本文能帮助复刻视觉意图，但不能保证逐像素复现。后续新增资产应如实填写可取得的信息；未取得的字段标记“未记录”，不得推测补造。
...
- 模型版本或快照：已记录／未记录。
- 生成日期：
- 参考图与 mask 版本：已记录／未使用／未记录。
- seed 与其他可复现参数：已记录／工具未提供／未记录。
- 尺寸：
- Alpha：
